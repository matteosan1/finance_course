...
\begin{frame}{Numeraires}
  \begin{itemize}
  \item Ideally we would like to define a probability measure $\mathcal{Q}$ under which the discounted derivative process IS a martingale, such that the expectation of the payoff is analytically tractable (i.e. easy to compute).
  \item The question that arises is: how can we determine such measure $\mathcal{Q}$ ?
...
\begin{frame}{Numeraires}
  \begin{itemize}
  \item We may compute values w.r.t. USD or EUR or JPY.
  \item Of course, others mught prefer use commodities: 1 OZ of gold could be a numeraire.
  \item Clearly, once we choose a numerarie e.g. 1 USD, we determine the value of other assets:
    \begin{center}
      \includegraphics[width=0.4\linewidth]{usd_eur}
      \includegraphics[width=0.4\linewidth]{usd_chy}
    \end{center}
  \end{itemize}
\end{frame}
...
\begin{frame}{Numeraires}
  ...
  \item In particular a change of measure implies also a change in the measure (probability distribution). Indeed with the bank account as numeraire
    %it has to be determined a new probability measure under which the transformed process $\tilde{S_t}$ will be a martingale (changing the numeraire implies a change of the probability measure).
    \begin{equation*}
      \begin{cases}
...
\begin{frame}{Asset Price divided by Numeraire}
  ...
  From the Change of Numeraire theorem we can define a new measure by
  \begin{equation*}
  ...
  \end{equation*}
  So $S/N$ is a $\mathcal{Q}^N$ martingale.
\end{frame}
...
\begin{frame}{A Useful Separation}
  \begin{itemize}
  \item Until now we have used $B(t)$, the money market account, as numeraire. But as we have seen it is natural to look for the most convenient numeraire, which minimizes the mathematical difficulties according to the problem at hand.
...
\begin{frame}{Girsanov Theorem}
  ...
  \item We therefore want to choose the Radon-Nikodym derivative so that the drift of $W_t$ w.r.t. $\mathcal{Q}$ exactly cancels out the drift of $S_t$, leaving us with a pure diffusion process (martingale). 
  \end{itemize}
\end{frame}

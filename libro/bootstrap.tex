...
Finally it is useful to put the expectation in terms of discount factors
...
\textbf{Our ultimate goal is to take a series of Overnight Index Swap quotations, and determine the discount factors implied by their prices.} 
...
This class goes into \texttt{finmarkets.py} library as a useful tool to work with Overnight Index Swaps.
\end{finmarkets}
...
At the beginning of the Chapter we said that the OIS market quotes represent the \emph{fair} prices (i.e. the price which buyer and seller happily agree to pay for the contract). This means that the OIS NPV's should be close to 0 hence the natural choice for the objective function is
\begin{equation}
\mathrm{Objective Function} =	\sum_{i=1}^{n}\mathrm{NPV}^\mathrm{OIS}_i(\mathcal{C})^2
\end{equation}
\noindent 
so the NPVs are the $f_i$ of Eq.~\ref{eq:bootstrap_as_minimization}.
...
In the previous examples the number of minimization parameters,  \emph{the degrees of freedom} of the problem, were clear:
\begin{itemize}
\item 1 for the can example, the can radius;
\item 2 for the fence problem, width and height of the field.
\end{itemize}

A discount curve is characterized by a set of pillar dates ($\mathbf{d}$) and a set of discount factors ($\mathbf{x}$), although it hasn't yet been identified any constraint on how many points the curve has to be made of (too many or too few points may prevent us from finding the solution).

In practice \textbf{it makes sense to choose the degrees of freedom to match the number of market quotes}. In particular it is wise to choose the pillar dates of the discount curve equal to the set of the swap expiry dates, leaving as unknown parameters just the discount factors.
...
The first to implement is the creation of the swap objects according to all the available market quotes
...
\tightlist
\item
  define the objective function: the sum of the squared NPVs of the OIS (today's discount factor is fixed to 1 so it may not be included in the minimization parameters). Note that any objective function could have constant additional parameters beside those used in the minimization, the convention assumes that the first parameter of the function must be that used to find the minimum. 
...
\end{ipython}
\begin{ioutput}
      fun: 7.524412227760359e-11
 hess_inv: <34x34 LbfgsInvHessProduct with dtype=float64>
      jac: array([ 3.32621854e-05,  2.86254121e-10, -3.41693024e-10, 
                  -2.00079832e-09, -4.76311694e-09, -8.17657833e-09,
                  ...
                   5.53307407e-06,  1.15870053e-07, -6.00554370e-06])
  message: b'CONVERGENCE: NORM_OF_PROJECTED_GRADIENT_<=_PGTOL'
     nfev: 245
      nit: 5
   status: 0
  success: True
        x: array([1.        , 1.00030147, 1.00058831, 1.00089012, 1.00119726,
                  1.0014702 , 1.00177764, 1.00207127, 1.00237486, 1.00266883,
                  1.00297278, 1.00326755, 1.00355768, 1.00445928, 1.00530914,
                  1.00614223, 1.00692722, 1.00906709, 1.00931689, 1.0070987 ,
                  1.00189684, 0.99379161, 0.98332933, 0.97101021, 0.95723227,
                  0.94267596, 0.92772621, 0.88312958, 0.81780977, 0.76554631,
                  0.71988653, 0.64350803, 0.59281895, 0.54546952])
\end{ioutput}
\end{itemize}

A useful cross-check to perform to understand if everything went fine, is the comparison of the objective function value at the beginning and at the end of the minimization.
...
\end{ipython}
\begin{ioutput}
Initial objective function value 1.0558491183367456
Final objective function value 7.524412227760359e-11
\end{ioutput}
The objective function at the end of the minimisation is not exactly 0 (and rarely it will be) but its value is small enough for us to be satisfied: the initial value was around $1$ and now it is $10^{-11}$ so several orders of magnitude smaller. This means that with the derived discount curve the NPV's of our OIS won't be identically 0 but so small that can be regarded as they would.

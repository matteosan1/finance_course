...
%), if 0 ≤ x ≤ 1;
%0, otherwise.
%Find k, together with the expectation and the variance of the random
%variable Y defined as Y = 3X − 1.
%
%$$ f(X) = k(1-x^2) $$
%
%$$Y = 3X - 1$$
...
%
%$$f(X=1) = \int_{0}^{1}k(1-x^2)dx = 1$$
%
%$$f(X=1) = k\left(x-\frac{1}{3}x^{3}\right|_{0}^{1}= k\left(1-\frac{1}{3}\right) = 1$$
%
%$$k = \frac{3}{2}$$
%
...
%
%
%$$Var(Y) = \mathbb{E}[Y^2] - (\mathbb{E}[Y])^2 = \frac{3}{2}\int_{0}^{1}(9x^2 - 6x + 1)(1-x^2)dx - \frac{1}{64} =  \frac{3}{2}\int_{0}^{1}(- 9x^4 + 6x^3 + 8x^2 - 6x + 1)dx - \frac{1}{64} = \frac{3}{2}\left(- \frac{9}{5}x^5 + \frac{6}{4}x^4 + \frac{8}{3}x^3 - 3x^2 + x\right|_{0}^{1} - \frac{1}{64} = \frac{3}{2}\left(- \frac{9}{5} + \frac{6}{4} + \frac{8}{3} - 3 + 1\right) - \frac{1}{64} = \frac{11}{20} - \frac{1}{64} = \frac{171}{320}$$
%
%
\begin{question}
...
%f (x) = x^2/ 9000, if 0 <x <30,
%f (x) = 0 otherwise.
%
%a) Calculate the probability that the client has a daily spending between 15 and 20 euros.
%b) What is his/her expected daily expenditure? Calculate the variance of spending.
%c) Next winter the department store is granting the following "discount vouchers", whose
%amount depends on the client's expenditure:
%* 1 euro for spending between 10 and 20 euros,
%* 1.5 euros for spending between 20 and 25 euros,
%* 3 euros if the expense exceeds 25 euros,
%Derive the distribution and the expected value of the discounts obtained by the client.
%d) The customer has paid for his/her purchases in the store with a credit card which applies a
%4% charge. What is the probability that the customer has paid (with charges included) between 10 and 15 euros?
%
%
%
%def F(x):

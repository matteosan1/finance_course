...
for i in range(1, len(df)-1):
    rets.append(w[0]*df.iloc[i]['aapl_rets']*df.iloc[-1]['aapl'] + 
...
\begin{ipython}
scaled = df[['aapl_rets', 'nflx_rets']].iloc[1:-1]*df[['aapl', 'nflx']].iloc[-1].values
rets2 = scaled.dot(w)

print('Historical VAR is {:.3f}'.format(np.percentile(rets2, 5)))
\end{ipython}
\begin{ioutput}
Historical VAR is -12.368
\end{ioutput}
...
In this case a t-student and a normal Gaussian have been tried and the former resulting quantiles are in good agreement with the historical estimate.
...
np.random.seed(1)
n_sims = 100000
sim_var_returns = mvnorm.rvs(n_sims)
sim_returns = sim_var_returns*df[['aapl', 'nflx']].iloc[-1].values
p_returns = sim_returns.dot(w)
mc_var = np.percentile(p_returns, 5)

print('Simulated VAR is {:.3f}'.format(mc_var))
\end{ipython}
\begin{ioutput}
Simulated VAR is -13.679
\end{ioutput}

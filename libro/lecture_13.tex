...
multiple counter-parties in a specific portfolio.

The exposure $\textrm{EE}(\tau)$ is defined as the sum 
...
With respect to the Value at Risk, the time horizon is usually one year 
and the percentile is the $99.9^{th}$, so it returns the loss that is 
exceeded only in 1 case out of 1000. 

Credit Var is actually either the difference of the percentile from the mean,
or the percentile itself. There is more than one possible definition but we 
will use the latter.
...
The portfolio is priced in every simulated scenario of the basic financial variables, including defaults, obtaining a number of scenarios for the portfolio value at the risk horizon.

It is then straightforward to derive the Credit VaR from the distribution of the losses.

Consider a portfolio of twenty zero coupon bonds each one with a default probability for the next year of 8\% and the same face value  (\euro{100}). The recovery rate in case of default is $R=40\%$ and the risk free rate is 1\%.
...
\PY{n}{N} \PY{o}{=} \PY{p}{[}\PY{l+m+mi}{100} \PY{k}{for} \PY{n}{\PYZus{}} \PY{o+ow}{in} \PY{n+nb}{range}\PY{p}{(}\PY{l+m+mi}{20}\PY{p}{)}\PY{p}{]}
...
\PY{k}{for} \PY{n}{\PYZus{}} \PY{o+ow}{in} \PY{n+nb}{range}\PY{p}{(}\PY{n}{scenarios}\PY{p}{)}\PY{p}{:}
    ...
    \PY{k}{for} \PY{n}{i} \PY{o+ow}{in} \PY{n+nb}{range}\PY{p}{(}\PY{n}{bonds}\PY{p}{)}\PY{p}{:}
        \PY{k}{if} \PY{n}{unif}\PY{p}{[}\PY{n}{i}\PY{p}{]} \PY{o}{\PYZgt{}} \PY{n}{ccs}\PY{p}{[}\PY{n}{i}\PY{p}{]}\PY{o}{.}\PY{n}{ndp}\PY{p}{(}\PY{n}{pillars}\PY{p}{[}\PY{o}{\PYZhy{}}\PY{l+m+mi}{1}\PY{p}{]}\PY{p}{)}\PY{p}{:}
            \PY{n}{loss} \PY{o}{+}\PY{o}{=} \PY{p}{(}\PY{l+m+mi}{1} \PY{o}{\PYZhy{}} \PY{n}{R}\PY{p}{)}\PY{o}{*}\PY{n}{N}\PY{p}{[}\PY{n}{i}\PY{p}{]}\PY{o}{*}\PY{n}{dc}\PY{o}{.}\PY{n}{df}\PY{p}{(}\PY{n}{pillars}\PY{p}{[}\PY{o}{\PYZhy{}}\PY{l+m+mi}{1}\PY{p}{]}\PY{p}{)} 
...
Indeed if you a have $n$ counter-parties with the same default probability $DP(T)$ the percentage of defaults at time $T$ is $DP$ itself ($\textrm{\% of defaults} = \textrm{n\_defaults}/n = n\cdot DP/n$).
...
Once the time $T$ as been chosen the only random variable appearing in the conditional default probability expression above is $M$, therefore we can be $X\%$ certain that the percentage of losses over $T$ years on a large portfolio will be \emph{less} than $V(X,T)$ where
\[
V(X,T)=\Phi\Big(\cfrac{\Phi^{-1}[Q(T)]+\Phi^{-1}(X)\sqrt{\rho}}{\sqrt{1-\rho}}\Big)
\]
When the confidence level $X\%$ is used and the time horizon is $T$, a rough estimate of the Credit VaR is therefore $L(1-R)V(X,T)$, where $L$ is the size of the portfolio and $R$ is the recovery rate.
...
\(\mathbb{E}[\text{max(0, NPV}_\text{portfolio})]\), and $dQ$ is the "instantaneous" probability of default.
...
face due to the possible default of some counter-party you are having
business with. Instead CVA measures the pricing component of this risk, i.e.
...
In case of zero coupon bond bonds the computation of the CVA can be simplified. Indeed in this case the exposure of the investor is equal to the face value of the bond, so it is enough to loop through each days from the observation date to the maturity of the bond and compute the CVA using Eq.~\ref{eq:cva_discrete}.

Imagine a 3-years zero coupon bond with a face value of $N=$~\euro{100}. 
...
To compute the CVA we need to first define a discount curve and the credit curve corresponding to the issuer, then we perform a daily loop. Finally the price of the bond is the default-risk-free price minus the CVA.
...
\PY{n}{V} \PY{o}{=} \PY{n}{N} \PY{o}{*} \PY{n}{math}\PY{o}{.}\PY{n}{exp}\PY{p}{(}\PY{o}{\PYZhy{}}\PY{n}{r}\PY{o}{*}\PY{n}{i}\PY{p}{)}
...
\PY{n}{cva} \PY{o}{*}\PY{o}{=} \PY{p}{(}\PY{l+m+mi}{1}\PY{o}{\PYZhy{}}\PY{n}{R}\PY{p}{)} \PY{o}{*} \PY{n}{N}
...
\PY{n+nb}{print} \PY{p}{(}\PY{l+s+s2}{\PYZdq{}}\PY{l+s+s2}{Bond Price: }\PY{l+s+si}{\PYZob{}:.2f\PYZcb{}}\PY{l+s+s2}{\PYZdq{}}\PY{o}{.}\PY{n}{format}\PY{p}{(}\PY{n}{V} \PY{o}{\PYZhy{}} \PY{n}{cva}\PY{p}{)}\PY{p}{)}
...
If we assume that the recovery payments are done at the maturity date of the zero coupon bond the formula of the CVA calculation can be simplified further, indeed

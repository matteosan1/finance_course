...
return and variance in return (i.e., risk), the latter measures the
...
\item
  the covariance matrix of returns (it characterizes not only the individual variability
...
series of the closing price of the last 5 yers.
...
From now on we indicate with $w_i$ the fraction of the portfolio devoted to stock $i$ 
(weight).
To simulate a portfolio it is enough to throw randomly five weights with the constraint that
they have to sum up to 1. 

In Figure~\ref{fig:mc_portfolio} a large number of simulated portfolios are shown in the 
return vs volatility plane. In this case no attempt of any optimization whatsoever has been made.
...
Markowitz model states that the weights should be chosen such that the 
portfolio volatility (or its variance) is minimised. 
So the application of Markovitz model reduces to
an optimisation problem: given the covariance matrix of the portfolio
$\Sigma$ estimated before from the historical series, we need to find:
...
The solution recommends to devote about 44\% of the portfolio be to AAPL,
...
to base this estimate on more current information about
...
0.45).
So in the code, beside the usual constraint on the sum of weights, we have to add the one that force the resulting return to be equal to the chosen target.

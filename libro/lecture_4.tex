...
CLASS\textsubscript{\(\rightarrow\)}collection of functions that operate
on some dataset\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.97\columnwidth}\centering
DATA ITEMS \(\rightarrow\) are called the \textbf{attributes} of the
class\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.97\columnwidth}\centering
CLASS INSTANCE \(\rightarrow\) a specific collection of data belonging
to the particular object we are representing\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.97\columnwidth}\centering
CLASS FUNCTIONS \(\rightarrow\) are called \textbf{methods} of the
class, and act on \textbf{instances}\strut
\end{minipage}\tabularnewline
\bottomrule
\end{longtable}
...
Examples of class are: a class representing a generic building (with number of entrances, number of floors, a flag to know if there is a garden...), a generic dog (with age, fur color...) or a generic computer (with manufacturer, RAM size, CPU type,...), see Fig.~\ref{fig:classes}.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.5\textwidth]{inheritance.png}

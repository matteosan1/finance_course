...
From the third one on it is not as simple to solve them analytically since involve third order (or more) equations. 
Anyway it is possible to solve them numerically.
Follows the table with the result up to to the 4 year rate:
...
Below the pythonic solution of the last equation:
...
manufacture a circular cylindrical can of volume, \(33~\mathrm{cm}^3\), see Figure~\ref{fig:cylinder}.
...
\[ S = 2\pi rh + 2\cdot(\pi r^2) = \cfrac{2\cdot 33}{r} + 2\cdot(\pi r^2)\]
...
about 1.74 cm (so clearly Coke doesn't care about costs of can
production).

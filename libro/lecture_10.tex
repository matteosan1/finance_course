...
not suffer a credit event \textbf{before} a given value date. Non-default probability is a cumulative probability.
...
sequence of NDPs. 
Keeping this parallel we will implement a \texttt{CreditCurve} class
that provides a method which interpolates these NDPs
to return the non-default probability at an arbitrary date.
...
\[\mathrm{NPV_{premium}} = \sum_{i=1}^{n} S \cdot D(d_i) \cdot N(d_i)\]

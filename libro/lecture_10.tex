...
practice we'll calculate it numerically, and therefore it'll be the, \emph{annualised}, conditional probability of the issuer defaulting between
...
\[\lambda = \cfrac{\mathbb{P}(A\cap B)}{\mathbb{P}(B)} = \cfrac{DP(\tau\in (t, t+dt))}{DP(\tau>t)} = \cfrac{\cfrac{d(1-N)}{dt}}{N(t_0, t_1)} = -\cfrac{dN}{dt}\cdot\frac{1}{N(t_0, t_1)}.\]
where $N$ and $DP$ are survival and default probabilities and $\tau$ indicates the time at which the credit event occurred. Also remember that $DP(t) = 1 - N(t)$ or that the probability of default at $t$ is equal to 1 minus the non-default probability.
...
\PY{n}{cc} \PY{o}{=} \PY{n}{CreditCurve}\PY{p}{(}
      \PY{p}{[}\PY{n}{observation\PYZus{}date}\PY{p}{,} \PY{n}{pricing\PYZus{}date} \PY{o}{+} \PY{n}{relativedelta}\PY{p}{(}\PY{n}{years}\PY{o}{=}\PY{l+m+mi}{2}\PY{p}{)}\PY{p}{]}\PY{p}{,}
      \PY{p}{[}\PY{l+m+mf}{1.0}\PY{p}{,} \PY{l+m+mf}{0.8}\PY{p}{]}
\PY{p}{)}

\PY{n}{cc}\PY{o}{.}\PY{n}{ndp}\PY{p}{(}\PY{n}{observation\PYZus{}date} \PY{o}{+} \PY{n}{relativedelta}\PY{p}{(}\PY{n}{years}\PY{o}{=}\PY{l+m+mi}{1}\PY{p}{)}\PY{p}{)}

0.9
\PY{n}{cc}\PY{o}{.}\PY{n}{hazard}\PY{p}{(}\PY{n}{observation\PYZus{}date} \PY{o}{+} \PY{n}{relativedelta}\PY{p}{(}\PY{n}{years}\PY{o}{=}\PY{l+m+mi}{1}\PY{p}{)}\PY{p}{)}
...
\item
  ...
  \textbf{loss given default} (LGD) if and when the credit event occurs, $F$ is the notional of the contract.
...
be evaluated when \(\tau > d_i\). Remember that if \(x\) is a random
variable with a finite number of finite outcomes
\(x_{1},x_{2},\ldots ,x_{k}\) occurring with probabilities
\(p_{1},p_{2},\ldots ,p_{k}\) respectively, the expectation of \(x\) is:
...
In our case \(x_i = F\cdot S\cdot D(d_i)\) and \(p_i=N(d_i)\) so the NPV of the
leg can be expressed as:

\[\textrm{NPV}_{premium} = F\cdot \sum_{i=1}^{n} S \cdot D(t_i) \cdot N(t_i)\]
...
Also in this case we are dealing with a probabilistic event so the expectation value has to be computed taking into account the payments $F(1-R)\times D(\tau)$ averaged over the default probability space $DP(\tau)$. 
...
\begin{align*}
\mathrm{NPV_{default}} &= \mathbb{E}[F(1-R) \times D(\tau) \times \mathbb{1}\{\tau \leq d_n\} ] \\
&= \sum_{d'=d_0}^{d_n} \mathbb{E}[ (F(1-R) \times D(\tau)) \mid \tau = d'] DP(\tau = d') \\
...
\PY{n}{credit\PYZus{}curve} \PY{o}{=} \PY{n}{CreditCurve}\PY{p}{(}\PY{p}{[}\PY{n}{observation\PYZus{}date}\PY{p}{,} 
\PY{n}{observation\PYZus{}date} \PY{o}{+} \PY{n}{relativedelta}\PY{p}{(}\PY{n}{months}\PY{o}{=}\PY{l+m+mi}{36}\PY{p}{)}\PY{p}{]}\PY{p}{,} 
\PY{p}{[}\PY{l+m+mf}{1.0}\PY{p}{,} \PY{l+m+mf}{0.7}\PY{p}{]}\PY{p}{)}

\PY{n}{cds} \PY{o}{=} \PY{n}{CreditDefaultSwap}\PY{p}{(}\PY{l+m+mf}{1e6}\PY{p}{,} \PY{n}{observation\PYZus{}date}\PY{p}{,} \PY{l+m+mf}{0.03}\PY{p}{,} \PY{l+m+mi}{3}\PY{p}{)}
...
\end{cases}\] where \(D\) is the proper discount factor. Since we don't

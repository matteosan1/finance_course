...
\[\lambda(t) = \cfrac{\mathbb{P}(A\cap B)}{\mathbb{P}(B)} = \cfrac{DP(\tau\in (t, t+dt))}{DP(\tau>t)} = \cfrac{\cfrac{d(1-N)}{dt}}{N(t_0, t)} = -\cfrac{dN}{dt}\cdot\frac{1}{N(t_0, t)}.\]
...
\[\lambda(t) = -\cfrac{1}{dt}\cdot\cfrac{dN}{N} = -\cfrac{d(\textrm{log}N)}{dt} \implies d(\textrm{log}N) = -\lambda dt \implies N(t_0, t) = e^{-\int_{t_0}^{t}\lambda(s) ds}\]
...
\item
  ...
  \textbf{loss given default} (LGD) if and when the credit event occurs, ($F$ is the notional of the contract, $R$ is a the \textbf{recovery value}).
\end{itemize}
...
\[\textrm{NPV}_{premium} = F\cdot \sum_{i=1}^{n} S \cdot D(d_i) \cdot N(d_i)\]

...
\end{equation}
(notice how we moved to infinitesimal variations in the limit $\Delta t\rightarrow 0$). A stochastic differential equation is a differential equation in which one or more of the terms is a stochastic process, resulting in a solution which is also a stochastic process.
...
Its relative change during a period \(dt\) can be then decomposed in two parts

\begin{itemize}
\tightlist
\item a deterministic part: that is the expected return from the stock held during the time period \(dt\) and which can be expressed as \(\mu S_tdt\) (with $\mu$ being the expected rate of return);
\item a stochastic part: which reflects the random changes of the market (e.g. as a response to external effects such as unexpected news). A reasonable assumption is to take this contribution proportional to the stock so \(\sigma S_t dW_t\), where \(dW_t\) is a Wiener process.
\end{itemize}
...
The solution of Eq.~\ref{eq:differential_relative} can be derived by applying It\(\hat{o}\)'s formula~\cite{bib:ito_lemma}, which roughly speaking says that the derivative of a stochastic function with respect to time has an additional \emph{deterministic} term with respect to the usual derivative we know from standard calculus. 
...
\begin{equation}
\begin{gathered}
\cfrac{\partial G}{\partial S} = \cfrac{1}{S_t}\\
\cfrac{\partial G}{\partial t} = 0\\
...
\begin{equation}
\begin{gathered}
d(\textrm{log} S_t) = \left(\mu S_t \cfrac{1}{S_t} + \cfrac{1}{2}\sigma^2 S_t^2 (-\cfrac{1}{S_t^2})\right)dt + \sigma Z\sqrt{dt}\\
d(\textrm{log} S_t) = \textrm{log} (S_t) - \textrm{log} (S_{t-1}) = \textrm{log} \cfrac{S_t}{S_{t-1}} = \left(\mu - \cfrac{1}{2}\sigma^2\right)dt + \sigma z\sqrt{dt}\\
...
\begin{equation}
d(\textrm{log} S_t) = \left(\mu - \cfrac{1}{2}\sigma^2\right)dt + \sigma Z\sqrt{dt}
\end{equation}
...
\begin{equation}
\textrm{log}S_t - \textrm{log}S_0 \approx\mathcal{N}\left[\left(\mu-\frac{\sigma^2}{2}\right)T, \sigma^2 T\right]
\end{equation}
...
The model we have just developed implies that the stock price at time T, given today's price, is log-normally distributed. One of the nicer properties of a log-normal distribution is to be positive definite and that's why log-normality is an important characteristic for stock prices: we need to \emph{ensure} that will never be negative. 
...
Note that both the drift $\mu$ and the diffusion $\sigma$ are functions of the process variable $X$ and the time $t$. Suppose that we are interested in simulating values of $X(T)$ without knowing an explicit solution of Eq.~\ref{eq:sde}. 
...
The Euler scheme is the simplest and probably the most used discretization scheme. In this kind of simulation the process is discretized at constant time steps $\Delta t$ denoted by $b$  as $\{X_b, X_{2b},\ldots, X_{mb}\} = \{X(t_i), X(t_{i+1}),...,X(t_{i+n})\}$. Furthermore, the variable $m$ indicates the total number of simulated time steps, i.e. $mb = T$.
...
In a first step, simulate one path for the asset price as it is done before and calculate, according to Eq.~\ref{eq:call_payoff}, the corresponding payoff. Repeat this step numerous times which provides you with several payoffs. As a last step, compute the mean of the risk-free discounted payoff. 

The fact that this Monte Carlo estimate is indeed consistent with the Black-Scholes formula results is illustrated in Fig.~\ref{fig:error_BS}. There the development of the relative pricing error, computed as $(C_n − C)/C$, with a steadily growing sample $n$ is shown. Note that the pricing error is gradually decreasing and eventually converging to 0 as $n$ goes to infinity. This is another proof that Monte Carlo provides correct results in the limit $n\rightarrow\infty$.
...
A \emph{Markov chain} is a mathematical system usually defined as a collection of random variables, that transition from one state to another according to certain probabilistic rules. These set of transition satisfies the Markov property, which states that the probability of transitioning to any particular state is dependent solely on the current state and not on the sequence of states that preceded it. 

Markov Chains are widely employed in economics, game theory, communication theory, genetics and finance. When it comes to real-world problems, they are used to study cruise control systems in motor vehicles, queues or lines of customers, exchange rates of currencies, etc\ldots

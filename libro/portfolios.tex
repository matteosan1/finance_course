...
Using an approach similar to that used in the previous Section it can be assessed the impact of the Covid19 pandemic on the market, clustering together assets that were most/least affected without any prior knowledge of their fundamental.
This analysis involves the stocks included in the S\&P500 in the time period between $1^{st}$ of January 2020 and $30^{th}$ of June 2021.

Let's look at the first principal component and select the stocks that have the most and least negative PCA weights, as shown in Fig.~\ref{fig:pca_covid_neg}.

%\begin{figure}[htb]
%	\centering
%	\includegraphics[width=.7\textwidth]{figures/pc1_weights_covid_neg}
%	\caption{Ten most negative (top) and least negative (bottom) weights of the PC1 portfolio.}
%	\label{fig:pca_covid_neg}
%\end{figure}

Notice from the figure above how the most negative stocks are in the tourism and the energy sector. This makes sense since Covid19 heavily impacted the traveling business, as well as the energy companies that provide fuels for those business. On the other hand, the least impacted companies fall into the consumer good sectors, which also makes sense since this sector benefited fro the boost in sales consumer goods due to the quarantine measures.
...
Note that this portfolio is formed with look-ahead bias, indeed portfolio weights are computed using future date that were not available at the time of market downturn. The PCA used in this way is therefore a backward looking analytics tool.

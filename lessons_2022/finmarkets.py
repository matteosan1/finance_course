...
from scipy.stats as ss#import norm, binom
...
class DiscountCurve:
    ...
    def yield(self, d):
        """
        yield: computes the annualized yield at a given date.
...
def call(St, K, r, sigma, ttm):
    return (St*norm.cdf(d_plus(St, r, sigma, ttm))-
        K*np.exp(-r*(ttm))*ss.norm.cdf(d_minus(St, r, sigma, ttm)))

def put(St, K, r, sigma, ttm):
    return (K*np.exp(-r*(ttm))*ss.norm.cdf(-d_minus(St, K, r, sigma, ttm))-
            St*norm.cdf(-d_plus(St, r, sigma, ttm)))
    
def d_plus(St, K, r, sigma, ttm):
    num = math.log(St/K) + (r + 0.5*sigma**2)*(ttm)
    den = sigma*math.sqrt(ttm)
    return num/den

def d_minus(St, K, r, sigma, ttm):
    return d_plus(St, K, r, sigma, ttm) - sigma*math.sqrt(ttm)
...
class InterestRateSwaption:
    ...
    def payoffBS(self, obs_date, dc, fc):
        ...
        dp = (log(S/K) + 0.5*self.sigma**2*T)/(self.sigma*sqrt(T))
        dm = (log(S/K) - 0.5*self.sigma**2*T)/(self.sigma*sqrt(T))
    ...
    def payoffMC(self, obs_date, dc, fc, n_scenarios=100000):
        ...
        for _ in range(n_scenarios):
            S = S0 * exp(-self.sigma**2/2*T + self.sigma*np.random.normal()*np.sqrt(T))
...
class ExpDefault(ss.rv_continuous): 
    def __init__(self, l):
        super().__init__()
        self.ulim = 100
        self.l = l
...
class BasketDefaultSwapsOneFactor:
    ...
    def one_factor_model(self, M, f, Q_dates, Q, dc, j):
        P = ss.norm.cdf((norm.ppf(Q) - np.sqrt(self.rho)*M)/np.sqrt(1-self.rho))
        b = binom(self.names, P)
        S = 1 - (1 - b.cdf(j-1))
        cc = CreditCurve(Q_dates, S)
        return f(dc, cc)*ss.norm.pdf(M)

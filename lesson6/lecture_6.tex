...
\documentclass[11pt]{article}
    ...
    % Document parameters
    % Document title
    \title{Interest Rate Derivatives - Practical Lesson 6}
...
\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}1}]:} [datetime.date(2019, 10, 28),
...
fixed rate \(S\) of the IRS, also known as the swap rate. \(S\) is the
value of K which makes \(\mathrm{NPV}(d)=0\).
...
IRS as follows:
  ...
  \begin{align*}
    \mathrm{NPV}(d; K) & = \mathrm{NPV}_{\mathrm{float}}(d) - \mathrm{NPV}_{\mathrm{fixed}}(d; K) = & \\ \\ &= \underbrace{\mathrm{NPV}_{\mathrm{float}}(d) - \mathrm{NPV}_{\mathrm{fixed}}(d; S)}_{\mathrm{=\;0}} + \mathrm{NPV}_{\mathrm{fixed}}(d;S) - \mathrm{NPV}_{\mathrm{fixed}}(d;K) & \\ & = N\cdot(S-K)\cdot\underbrace{\sum_{i=1}^{p}D(d, d_{i}^{\mathrm{fixed}})}_{\mathrm{'annxuity'}}
  \end{align*}
...
\end{Verbatim}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}4}]:} \PY{n}{irs} \PY{o}{=} \PY{n}{InterestRateSwap}\PY{p}{(}\PY{n}{pricing\PYZus{}date}\PY{p}{,} \PY{l+m+mf}{1e6}\PY{p}{,} \PY{l+m+mf}{0.05}\PY{p}{,} \PY{l+m+mi}{6}\PY{p}{,} \PY{l+m+mi}{4}\PY{p}{)}
        \PY{n}{irs}\PY{o}{.}\PY{n}{npv}\PY{p}{(}\PY{n}{discount\PYZus{}curve}\PY{p}{,} \PY{n}{libor\PYZus{}curve}\PY{p}{)}
\end{Verbatim}

\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}4}]:} -160130.58128473637
\end{Verbatim}
...
leg\ldots{})}\label{can-you-guess-what-could-be-the-swap-rate-given-that-the-npv-is-negative-remember-that-we-are-looking-at-this-contracts-from-the-point-of-view-of-the-receiver-of-the-floating-leg}}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}5}]:} \PY{n}{irs}\PY{o}{.}\PY{n}{swap\PYZus{}rate}\PY{p}{(}\PY{n}{discount\PYZus{}curve}\PY{p}{,} \PY{n}{libor\PYZus{}curve}\PY{p}{)}
\end{Verbatim}

\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}5}]:} 0.010254255993254184
\end{Verbatim}
...
{\color{incolor}In [{\color{incolor}6}]:} \PY{n}{irs2} \PY{o}{=} \PY{n}{InterestRateSwap}\PY{p}{(}\PY{n}{pricing\PYZus{}date}\PY{p}{,} \PY{l+m+mf}{1e6}\PY{p}{,} \PY{l+m+mf}{0.0102542}\PY{p}{,} \PY{l+m+mi}{6}\PY{p}{,} \PY{l+m+mi}{4}\PY{p}{)}
        \PY{n}{irs2}\PY{o}{.}\PY{n}{npv}\PY{p}{(}\PY{n}{discount\PYZus{}curve}\PY{p}{,} \PY{n}{libor\PYZus{}curve}\PY{p}{)}
\end{Verbatim}

\begin{Verbatim}[commandchars=\\\{\}]
{\color{outcolor}Out[{\color{outcolor}6}]:} 0.2255897471446154
\end{Verbatim}
...
inputs a \emph{seed} which determines univokely the serie. This means
...
\item
  ...
  \texttt{min} and \texttt{max} (uniform probability);
\item
  \texttt{sample(aList,\ k=n)} samples n elements from a the list
  \texttt{aList}.
\end{itemize}
...
['c', 'a']
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}10}]:} \PY{n}{numbers} \PY{o}{=} \PY{p}{[}\PY{p}{]}
...
Let's try to check the probability to get two kings drawing two cards
...
Let's try with a Monte Carlo simulation.
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}11}]:} \PY{k+kn}{from} \PY{n+nn}{random} \PY{k}{import} \PY{n}{sample}\PY{p}{,} \PY{n}{choices}\PY{p}{,} \PY{n}{seed}
...
distributions. Below an example with numpy.random which allows to throw
random numbers according to a normal distribution
(\(\mathcal{N}(0, 1)\)).

    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}12}]:} \PY{k+kn}{from} \PY{n+nn}{numpy}\PY{n+nn}{.}\PY{n+nn}{random} \PY{k}{import} \PY{n}{normal}
         \PY{k+kn}{from} \PY{n+nn}{matplotlib} \PY{k}{import} \PY{n}{pyplot} \PY{k}{as} \PY{n}{plt}
...
\item
  a stochastic part which reflects the random changes in stock price
...
where \(\epsilon\approx\mathcal{N}(0,1)\). To perform the simulation:
...
\end{itemize}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}14}]:} \PY{c+c1}{\PYZsh{} we\PYZsq{}ll need numpy.mean and numpy.std to calculate the average and standard }
...
Swaption NPV: 3351.42 EUR (+/- 66.14 EUR with 99\% confidence)
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
{\color{incolor}In [{\color{incolor}15}]:} \PY{c+c1}{\PYZsh{} Note that this is not *strictly speaking* the correct way of calculating}
         ...
         \PY{c+c1}{\PYZsh{} However, it\PYZsq{}s simpler to calculate it as above and it doesn\PYZsq{}t}
         \PY{c+c1}{\PYZsh{} make any difference to the result, since }
...
\begin{figure}[ht]
\centering
\includegraphics[width=.6\linewidth]{Standard_deviation_diagram.svg.png}
...
it to the \texttt{finmarkets} module.
...
\(\href{https://repl.it/@MatteoSani/support6}{curve\_data.py}\). Determine the value of an option to pay a fixed rate

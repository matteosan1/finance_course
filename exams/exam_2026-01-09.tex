...
\part $VaR_{95\%}$ for a portfolio $P=A+B$. Check if the sub-additivity property is fullfilled;
\part 95\% c.l. Expected Shortfall (ES) for each asset separately and portfolio $P$. Compare the results with parts (a) and (b).
\end{parts}
...
VaR only tells us the "threshold", both portfolios have a 95\% VaR of \$0. A risk manager looking only at VaR would see these portfolios as identical in risk.
...
\begin{equation*}
VaR_{95\%}(P) \ge VaR_{95\%}(A) + VaR_{95\%}(B)
\end{equation*}

VaR violets sub-additivity. The risk of losses increases diversifying the portfolio, which is in contrast with basic financial principles.

\part ES is the average loss in the worst $(1-\alpha)\%$ quantile. For the single assets we get

\begin{gather*}
	ES(A) = \frac{0.04\cdot 100 + 0.01\cdot 0}{0.05} = \$80 M \\
	ES(B) = \frac{0.04\cdot 500 + 0.01\cdot 0}{0.05} = \$400 M \\	
\end{gather*}
...
\begin{equation*}
	ES(P) = \frac{0.0484\cdot 300 + 0.0016\cdot 600}{0.05} = \$309 M 
\end{equation*}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\question 
Bank BestBankEver owns a portfolio containing $n=5$ loans. The portfolio is split into two classes:
...
\begin{solution}
\begin{parts}
	\part In case of independent assets the default probability is:
		\begin{equation*}
			P_{\text{def}} = 0.01^2\cdot 0.05^3 = 1.25\cdot 10^{-8}
		\end{equation*}  
		which represents a super-rare event.
	\part If there is perfect correlation all assets defaults at once, but since there are two levels of probability this global event is driven by the stronger assets (low risk):
		\begin{equation*}
	...
	\part By increasing the number of \textit{safer} assets the bank is not reducing the risk in the particular bad scenario with $\rho$ close to 1.
	Indeed following the reasoning of part (b) the default probability is still given by $P_{\text{def}} = \min(P_{low}, P_{high}) = 0.01$ so the action
...
\part Which action should the manager take if she needs to reduce to a 10\% level the uncertainty ?
\end{parts}
...
\end{align}
This demonstrates that estimating rare events (tails) requires a massive amount of computational data to be reliable.
\end{parts}	
\end{solution}
...
\question
You are given the following liquid instruments in the market. All bonds pay coupons annually, and the face value is \$100.
...
\begin{equation*}
	97=\frac{100}{(1+r_1)^1} \implies 1+r_1 = \frac{100}{97} \implies r_1 \approx 3.093\%
\end{equation*}
	Year 2 Spot Rate ($r_2$), using the result from $r_1$ to value Bond B:		
	\begin{equation*}
		101=\frac{5}{1.03093}+\frac{105}{(1+r_2)^2} \implies r_2 = 4.5\%
	\end{equation*}
	Year 3 Spot Rate ($r_3$):
	\begin{equation*}
		102=\frac{6}{1+r_1}+\frac{6}{(1+r_2)^2}+\frac{106}{(1+r_3)^3} \implies r_3 \approx 5.34\%		
	\end{equation*}
\part Because bootstrapping is a recursive process, an error in the Year 1 rate "contaminates" the Year 2 rate, which then compounds into the Year 3 rate. Small mispricings in short-term liquid instruments can lead to wildly unrealistic long-term forward rates.
\part While the spot curve might look smooth with linear interpolation, the resulting Forward Curve (the rate for the period between Year 2 and 3) will have "kinks" or "steps." In finance, we assume rates change continuously; a "steppy" forward curve creates arbitrage opportunities in the model that don't exist in reality.
\end{parts}
\end{solution}
...
\begin{itemize}
\item scenario A (Bull): $S1=\$110$;
\item scenario B (Base): $S1=\$80$;
\item scenario C (Bear): $S1=\$50$.
\end{itemize}

Calculate the "Expected Exposure" (EE) of the bank at maturity and the CVA for this 1-year contract.

Suppose the counterparty is an oil producer. If oil prices crash to \$50 (Scenario C), the counterparty is more likely to default. How does this affect the CVA calculation in this exercise?

If the oil price goes to \$50, the bank owes the counterparty money. What is the name of the adjustment the counterparty should make?

%\textbf{Hint}: the bank only faces credit risk if the contract has a positive value.
\makeemptybox{10cm}

\begin{solution}
Calculate the mark-to-market for each scenario:
\begin{itemize}
...
In this exercise, we assumed PD is independent of the oil price. If WWR exists, the PD would increase when the MtM is high. However, for an oil producer, Scenario C (MtM=-\$30,000) is when they are likely to default, but the bank's exposure is zero. WWR occurs when exposure and PD are positively correlated. If the counterparty were an oil consumer (like an airline), Scenario A would create WWR.

The adjustment for the bank's own default risk is called DVA (Debit Valuation Adjustment). The total adjustment is BCVA=CVA-DVA.
\end{solution}
...
\begin{parts}
\part Using the \textit{Credit Spread Approximation}, calculate the market-implied Hazard Rate ($\lambda$) for the counterparty. \textbf{Hint:} $s\approx \frac{\lambda}{(1-R)}$.

\part Calculate the probability that the counterparty defaults within the 1-year period ($PD_{0,1}$) using the exponential distribution formula: $PD=1-e^{-\lambda T}$.  (For a pen-and-paper exam, you can allow students to use the linear approximation $PD\approx \lambda\cdot T$ if calculators are restricted).
...
\begin{align*}
CVA=LGD\cdot PD\cdot EE \\
CVA=0.60\cdot 0.05\cdot 2,000,000 \\
CVA=0.03\cdot 2,000,000=\$60,000
\end{align*}
\part Calculate the new CVA.
If $s$ increases by 1.5x (from 3\% to 4.5\%), and $LGD$ and $EE$ stay constant, the CVA also increases by 1.5x. New $CVA=\$60,000\cdot 1.5=\$90,000$.
Total change in CVA = \$30,000. Total change in bps = 150 bps.
\begin{equation*}
...
\item time-horizon ($T$): 0.1 year (with 2 step: $\Delta t= 0.05$).
\end{itemize}

\begin{parts}
\part Using the Euler scheme compute $r_1$ and $r_2$ at the two time steps. The stochastic component can be computed using the following sample from the normal distribution $Z\approx \mathcal{N}(0,1)$:
\begin{itemize}
\item Step 1 ($t=0.05$): $z_1=1.0$;
\item Step 2 ($t=0.10$): $z_2=-1.5$.
\end{itemize}

\part What would it happen if the mean reversion speed was very high (e.g. $\kappa = 50$) keeping $\Delta t=0.05$ ?
...
\begin{align*}
\Delta r= 2(0.05-0.07)(0.05)+(0.02)0.05(1.0) \\
\Delta r= -0.002+0.00447=0.00247 \\
r_1=0.07+0.00247=7.247\%
\end{align*}
\item Solution step 2 ($r_2$):
\begin{align*}
\Delta r = 2(0.05-0.07247)(0.05)+(0.02)0.05(-1.5) \\
\delta r =-0.00225-0.00670=-0.00895 \\
r_2=0.07247-0.00895=6.352\%
\end{align*}
\end{itemize}
\part If $\kappa\Delta t >1$, the deterministic term ($1-\kappa \Delta t$) becomes negative, forcing the solution to oscillate aroung the mean. If $\kappa \Delta t >2$, Euler scheme becomes unstable and the short rate diverges. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\section*{Dati del Mercato}
	Si considerino le seguenti informazioni relative al mercato finanziario in equilibrio:
	\begin{itemize}
		\item Tasso privo di rischio ($R_f$): $3\%$
		\item Rendimento atteso del portafoglio di mercato ($E[R_m]$): $10\%$
		\item Varianza del mercato ($\sigma^2_m$): $0,04$
	\end{itemize}
	
\question 
Conside the following market data:
\begin{itemize}
...
\begin{parts}
\part Using the CAPM compute the expected return of asset A, knowing its $\beta_A = 1.5$.
\part Consider asset B, which currently has a return of $8,6\%$. From a historical analysis the asset has $\beta_B = 0,9$. Determine if the asset is correctly valued, undervalued or overvalued using Jensen-alpha ($\alpha$).
...
\begin{solution}
A payer IRS value can be seen as the difference between a fixed rate bond and a variable rate bond:
\[ NPV = V_{float} - V_{fix} \]
			
Since the Libor rate is equal to the discounting rate the floating leg equals par:
...
\part Determine the swap NPV for the party paying fixed and explainwhy the NPV is positive even if the short-term rate decreases.
\end{parts}
...
The NPV is positive because a 3y-swap has a greater duration for longer maturities. 2 and 3 years interest rate increase overcome the decrease at 1 year, globally increasing the swap value.
\wnd{parts}
\end{solution}

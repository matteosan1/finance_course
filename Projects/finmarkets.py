...
class CreditDefaultSwap:
  def __init__(self, notional, start_date, nyears, 
	       fixed_spread, tenor=3, recovery=0.4):
    self.notional = notional
    self.payment_dates = generate_dates(start_date, nyears*12, tenor)
  ...
  def premium_leg_npv(self, discount_curve, credit_curve):
    npv = 0
    for i in range(1, len(self.payment_dates)):
      npv += self.fixed_spread * discount_curve.df(self.payment_dates[i]) * credit_curve.ndp(self.payment_dates[i])
  ...
  def default_leg_npv(self, discount_curve, credit_curve):
    ...
    while d <= self.payment_dates[-1]:
      npv += discount_curve.df(d) * (credit_curve.ndp(d) - credit_curve.ndp(d + relativedelta(days=1)))	

        ...
        for i, S_t in enumerate(paths[:, sims]):
            for t in T:
                ...
                if i >= M:
                    continue
                for k in K:
                    call_price = call(S_t, k, r, sigmas[sims], t)
                    writer.writerow([sigmas[sims], k, t, S_t, call_price])
...
with open("bs_testing.csv", mode='w') as f:
    ...
    for i, S_t in enumerate(paths[:, 0]):
        call_price = call(S_t, K,  r, sigmas[0], T)
        writer.writerow([sigmas[0], K, T, S_t, call_price])

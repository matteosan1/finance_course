...
The Black-Litterman Model employs a Bayesian technique to integrate investor’s subjective views of expected returns on one or more assets with the market equilibrium vector (\emph{prior distribution}) of expected returns to obtain a new, mixed estimate of expected returns. The new vector of returns (the \emph{posterior distribution}) is a weighted complex average of the investor’s views and market equilibrium.
...
\item We integrate all the values computed previously in the Black-Litterman model. %Table 4 depicts the results obtained via the Black-Litterman allocation model.
...
\subsection{Black-Litterman Model Under a Bayesian Framework}

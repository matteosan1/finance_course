...
Note that the table contains also dates in which may be dividends (in this case there are none so the rows are \texttt{NaN}).
...
Of course the first "variation" value is \texttt{NaN} since there is no previous price to compare with. \texttt{NaN}, short for \emph{not a number}, represents the \texttt{null} value, it is the value that is given to missing fields in a row. 
...
Usually when importing data with \texttt{pandas} we may have some \texttt{NaN} values. Like for the outliers we can use the \texttt{replace} or \texttt{mask} methods to remove the \texttt{NaN}s. In case the whole row as \texttt{NaN} it may be wise to drop it entirely.

Additionally we can use \texttt{dropna()} which remove all the \texttt{NaN} in the dataframe at once.

...
\begin{equation*}
\Sigma = \begin{bmatrix}
\sigma^2 (X_0) & \text{Cov}(X_0, X_1) & \text{Cov}(X_0, X_2)\\
 \text{Cov}(X_1, X_0) & \sigma^2 (X_1) & \text{Cov}(X_1, X_2)\\
 \text{Cov}(X_2, X_0) & \text{Cov}(X_2, X_1) & \sigma^2 (X_2)\\
\end{bmatrix}
\end{equation*}
...
Imagine we want to transform a uniform distribution into Gaussian. The transformation takes uniform samples $u$between 0 and 1, interpreted as a probabilities, and then returns $F_{\textrm{normal}}^{-1}(u)$. The following code hows how this can be done in \texttt{python}. 

\pythoncode{code/modelling_risk_1.py}

We start by generating a sample from the uniform distribution using \texttt{scipy.stats.uniform}. We can then exploits the \texttt{numpy.array} functionality to avoid a lot of for-loop cycles. 
...
In the center plot of Fig.~\ref{fig:a_to_b_to_a} the Gaussian obtained with the code below is shown
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\textwidth]{figures/uniform_to_gauss}
...
A \emph{copula} $\mathcal{C}(F_1, F_2, \ldots, F_n, \rho)$ is a multivariate distribution which examines the association or the dependences between many variables. It allows the joining of multiple univariate distributions to a single multivariate distribution.
...
In this example we are going to consider the closing prices of two assets like BMW and Siemens in the period between 2010 and 2022. 
The corresponding daily returns can be found in \href{https://github.com/matteosan1/finance_course/raw/master/input_files/bmw_siemens.csv}{bmw\_siemens.csv}.
...
In the code we first load the historical series and check the correlation between the two assets.
\begin{ioutput}
          BMW.DE    SIE.DE
BMW.DE  1.000000  0.664654
SIE.DE  0.664654  1.000000
\end{ioutput}
Indeed the returns are correlated with a coefficient of about 0.6646 as shown in Figure~\ref{fig:bmw_sie_returns}.
...
By fitting the return histograms (e.g. using t-student~\cite{bib:t_student} functions) we can determine the parametric for the marginal distributions.
...
To model the joint probability distribution we have to:
\begin{itemize}
\tightlist
...
To keep thing simple we are going to assume a Gaussian correlation, this is not mandatory though, since we could have modeled it with different distribution either.
...
We then apply the \texttt{cdf} transformation to convert the marginal distributions to uniform and 
...
Note that to compute the probability, ignoring the correlation between the returns, we have just sampled from the two marginals and counted how many times they both are negative.
...
\end{finmarkets}
\pythoncode{code/modelling_risk_3.py}
...
\pythoncodenon{code/modelling_risk_4.py}
\begin{ioutput}
4778.843544161504
\end{ioutput}
\noindent
...
\pythoncode{code/modelling_risk_5.py}
...
\pythoncode{code/modelling_risk_6.py}
...
\pythoncode{code/modelling_risk_7.py}
...
\pythoncode{code/modelling_risk_8.py}

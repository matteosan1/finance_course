...
In other words, since in credit modeling, the PDF of the default probability $f(t)$ is defined as the rate of change of the cumulative default probability and $\Pdef(t_0, t) = 1 - \Psur(t_0, t)$, it follows that:
\begin{gather}
f(t) = \frac{d\Pdef}{dt} = \frac{d(1-\Psur)}{dt} = -\frac{d\Psur}{dt} \\
\lambda(t) = \frac{f(t)}{\Psur(t)}
\end{gather}
...
\subsubsection{Piecewise Constant Hazard Rate}
...
A second slightly more complicated implementation a the Poisson Process allows to handle this new kind of hazard rate.

\pythoncodenon{code/credit_risk_poisson_process_piecewise_constant.py}

\subsubsection{Calibration Example}

To calibrate the PiecewisePoissonProcess to a non-flat market curve, we need to find the set of $\lambda$ values that make the model's survival probabilities match the market's implied survival probabilities at specific pillar dates (e.g., 1Y, 3Y, 5Y).
...
def calibrate_credit_process(market_times, market_probs, initial_guess=0.02):
    ...
    def residuals(log_lambdas):
        ...
        temp_process = PiecewisePoissonProcess(temp_lambdas, times)
    ...
    return PiecewisePoissonProcess(np.exp(sol.x), times)
...
# Check 7-year survival probability (interpolated)
print(f"Prob at 7Y: {calibrated_model.survival_prob(7.0):.2%}")

Calibrated Lambdas: [0.01005034 0.02062148 0.03827004 0.04576831]
Prob at 7Y: 88.00%

2. Why this matters: Term Structure of Credit

When you run this calibration, you'll see that self.lambdas are no longer identical. This allows the model to "bend" the survival curve to fit market expectations.

    Inversion: If the 1Y hazard rate is higher than the 5Y rate, the market is pricing immediate distress.

    Contango: If the rates increase over time, the market sees the entity as safe now but potentially risky in the long run.

3. Example Usage

Suppose you have CDS market data that implies the following survival probabilities (usually derived from CDS spreads):
...
market_t = [1.0, 3.0, 5.0]
market_p = [0.98, 0.92, 0.85]

# Initialize and calibrate
model = PiecewisePoissonProcess()
calibrated_lambdas = model.calibrate(market_t, market_p)

print(f"Pillar Times: {market_t}")
print(f"Calibrated Lambdas: {calibrated_lambdas}")

# Check the fit at a non-pillar date (e.g., 2 years)
print(f"Survival prob at 2Y: {model.survival_prob(2.0)[0]:.4f}")

The "Jump" in the Density

Because the hazard rate is piecewise constant, the PDF (Probability Density Function) will have visible "jumps" at the pillar dates (1Y, 3Y, 5Y). While this looks strange on a graph, it is the industry standard because it makes the math of pricing instruments like CDS very fast and consistent.
...
\begin{finmarkets}
An alternative implementation of the default leg calculation (i.e. \texttt{npv\_default\_leg} method) is possible. The explicit daily loop over the lifetime of the swap could indeed be replaced by a numerical intergrato between $[t_0, T]$.
...
def npv_default_leg_quad(self, cc, dc):
    def integrand(t, cc, dc):
        current_date = self.payment_dates[0] \ 
                                  + relativedelta(days=int(t * 365))
        return dc.df(current_date) \
               * (cc.ndp(current_date) \
                  - cc.ndp(current_date + relativedelta(days=1)))    
    maturity_in_years = (self.payment_dates[-1] \
                         - self.payment_dates[0]).days / 365

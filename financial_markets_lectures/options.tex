...
An Asian option is a special option where the payoff is determined by \emph{the average underlying price over some pre-set period of time}; 
...
The Aritmethic average type instead needs to be estimated with Monte Carlo techniques.
...
As we have seen above an obvious way to make our estimate more precise is to increase the number $N$ of simuations (e.g. random price paths) considered. However this can be computationally expensive therefore different methods have been developed, namely \emph{variance reduction techniques}.
...
\begin{finmarkets}
The following class \texttt{AsianOption} implements all the methods described above besides tha analytical pricing formula of the Geometric average Asian option.
\end{finmarkets}
...
class AsianOption:
  ...
  def simulate_antithetic(self, n, timesteps=252, seed=10000):
    ...
    S = np.exp(-self.r*self.ttm) *\\
        (self.aritmethic_payoff(S1) + self.aritmethic_payoff(S2))/2
    price = np.mean(S)
